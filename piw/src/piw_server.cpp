...
void piw::server_t::tree_thunk(bct_server_t *s, bct_entity_t x, bct_data_t d)
{
    tsd_setcontext(x);
    server_t *r = (server_t *)s;
    ...
    try
    {
        piw::data_t d2 = piw::data_t::from_lent(d);
...
void piw::server_t::change_thunk(bct_server_t *s, bct_entity_t x, bct_data_t d)
{
    tsd_setcontext(x);
    server_t *r = (server_t *)s;
    ...
    try
    {
        ...
        if((r->flags_ & PLG_SERVER_RO)!=0)
        {
            return;
        }

        if((r->flags_ & PLG_SERVER_TRANSIENT)==0 && d2==r->data_)
        {
            return;
        }
