...
"* Bounds:\n"
"\n"
"These are the boundaries of the data that EigenD will generate. 'Lo' sets the lower limit, 'hi' set the higher limit and 'base' sets the starting point that corresponds to a neutral signal on the Eigenharp control. For instance, by setting 'base' to 50% for the 'Key Yaw' control, touching the key in its resting position, will generate the middle value of the parameter that you're controlling. Moving the key into either direction allows you to decrease or increase that middle value. Without increasing the base value above 0%, it wouldn't be possible to decrease the value by moving the keys, it would only be possible to increase it. By increasing the 'lo' value or decreasing the 'hi' value, you reduce the effective range of data that the Eigenharp control will generate, but you will still benefit from the full physical movement range of that control. This can allow you to for instance set the lower level on volume messages to prevent it from ever going to an inaudible value, or to make sure that it never becomes too loud.\n"
"\n"
