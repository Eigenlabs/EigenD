...
namespace piw
{
    class PIW_DECLSPEC_CLASS server_t: public pic::nocopy_t, public bct_server_t, virtual public pic::tracked_t
    {
        ...
        protected:
            ...
            virtual void server_change(const piw::data_t &d) { fchanged_(d); }
            virtual void server_tree(const piw::data_t &d) { }

        private:
            static bct_data_t attached_thunk(bct_server_t *, unsigned *);
            static void change_thunk(bct_server_t *s, bct_entity_t, bct_data_t);
            static void tree_thunk(bct_server_t *s, bct_entity_t, bct_data_t);
