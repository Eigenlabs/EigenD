...
struct pia_lnode_t: pia_server_t
{
    ...
    void server_closed();
    void server_set(const pia_data_t &);
    void server_tree(const pia_data_t &);
    ...
    static void job_data(void *n_, const pia_data_t &d);
    static void job_tree(void *n_, const pia_data_t &d);
    ...
    void ongoing();
    int receive_data(const unsigned char *, unsigned);
    int receive_tree(const unsigned char *, unsigned);
...
void pia_lnode_t::job_tree(void *n_, const pia_data_t & d)
{
    pia_lnode_t *n = (pia_lnode_t *)n_;
    bct_server_plug_tree(n->_client,n->entity()->api(),d.lend());
}

void pia_lnode_t::job_data(void *n_, const pia_data_t & d)
{
    pia_lnode_t *n = (pia_lnode_t *)n_;
    bct_server_plug_change(n->_client,n->entity()->api(),d.lend());
}

void pia_lnode_t::server_set(const pia_data_t &d)
{
    entity()->appq()->idle(cpoint_,job_data, this, d);
}

void pia_lnode_t::server_tree(const pia_data_t &d)
{
    entity()->appq()->idle(cpoint_,job_tree, this, d);
}
...
int pia_lnode_t::receive_tree(const unsigned char *msg, unsigned len)
{
    unsigned dl;
    const unsigned char *dp;
    int x;

    if((x=pie_gettsetlist(msg,len,&dp,&dl))<0)
    {
        return -1;
    }

    if((flags()&PLG_SERVER_LIST)!=0)
    {
        pia_data_t d = entity()->glue()->allocate_string((const char *)dp,dl);
        entity()->appq()->idle(cpoint_,job_tree, this, d);
    }
...
int pia_lnode_t::receive_data(const unsigned char *msg, unsigned len)
{
    int x;
    unsigned df;
    const unsigned char *dp;
    ...
    if((x=pie_getdata(msg,len,&df,&dl,&dp))<0)
    {
        return -1;
    }

    if((flags()&PLG_SERVER_RO)==0)
    {
    ...
    while((x=pie_getstanza(buffer,len,&bt,&p,&pl))>0)
    {
        ...
        if(nb)
        {
            ...
            switch(bt)
            {
                ...
                case BCTMTYPE_TREE_SET:
                    if((x=n->receive_tree(buffer,len))<0)
                    {
                        return;
                    }
                    PIC_ASSERT(len>=x);
                ...
                case BCTMTYPE_DATA_SET:
                    if((x=n->receive_data(buffer,len))<0)
                    {
                        return;
                    }
                    PIC_ASSERT(len>=x);
