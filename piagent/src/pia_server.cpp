...
struct cnode_t: pic::element_t<CNODE_CHILDREN>, pic::element_t<CNODE_RECEIVE>, pic::element_t<CNODE_SYNCS>, pic::element_t<CNODE_CLIENTS>, pic::element_t<CNODE_ROOTS>, pic::element_t<CNODE_SCLIENTS>
{
    ...
    static int api_change(bct_client_host_ops_t **, bct_data_t);
    ...
    static int api_child_change(bct_client_host_ops_t **, const unsigned char *, unsigned, bct_data_t);
    ...
    static int api_tree(bct_client_host_ops_t **, bct_data_t);
...
int cnode_t::api_tree(bct_client_host_ops_t **co, bct_data_t d)
{
    cnode_t *c = PIC_STRBASE(cnode_t,co,client_ops_);
    ...
    try
    {
        ...
        if(bct_data_type(d)!=BCTVTYPE_STRING)
        {
            return -1;
        }

        if(s && (s->flags_&PLG_SERVER_LIST)!=0)
        {
            s->server_tree(pia_data_t::from_lent(d));
            return 0;
        }
    }
    PIA_CATCHLOG_EREF(c->entity_)
...
int cnode_t::api_change(bct_client_host_ops_t **co, bct_data_t d)
{
    cnode_t *c = PIC_STRBASE(cnode_t,co,client_ops_);
    ...
    try
    {
        ...
        if(s && (s->flags_&PLG_SERVER_RO)==0)
        {
            s->server_set(pia_data_t::from_lent(d));
            return 0;
        }
    }
    PIA_CATCHLOG_EREF(c->entity_)
...
int cnode_t::api_child_change(bct_client_host_ops_t **co, const unsigned char *path, unsigned pathlen, bct_data_t d)
{
    cnode_t *c = PIC_STRBASE(cnode_t,co,client_ops_);
    ...
    try
    {
        ...
        if(s)
        {
            ...
            {
                if((t->flags_&PLG_SERVER_RO)==0)
                {
                    t->server_set(pia_data_t::from_lent(d));
                }
                return 0;
            }
        }
