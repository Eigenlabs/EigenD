...
#define BUFFER_SIZE    ((BCTLINK_MAXPACKET+48)*2)
...
namespace
{
    ...
    struct socket_t : pic::nocopy_t
    {
        socket_t(int f,int t, int p)
        {
            ...
            {
                pic::msg() << "can't open socket" << ": " << sys_errlist[errno] << pic::hurl;
            }
        }
    ...
    struct send_socket_t
    {
        send_socket_t(unsigned spc, unsigned long local,bool loop): socket_(AF_INET,SOCK_DGRAM,0), spc_(spc), local_(local), loop_(loop)
        {
            ...
            {
                pic::msg() << "Can't bind sender socket" << ": " << sys_errlist[errno] << pic::hurl;
            }
            ...
            {
                pic::msg() << "Can't setup looping" << ": " << sys_errlist[errno] << pic::hurl;
            }

            u_char ttl=loop?0:1;
            if(setsockopt(socket_.fd, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&ttl, sizeof(ttl)) < 0)
            {
                pic::msg() << "Can't setup mcast ttl" << ": " << sys_errlist[errno] << pic::hurl;
            }

            int ssz;
            socklen_t sszl = sizeof(ssz);

            if(getsockopt(socket_.fd,SOL_SOCKET, SO_SNDBUF, (char *)&ssz, &sszl)<0)
            {
                ssz=0;
            }
            ...
            if(ssz < BUFFER_SIZE)
            {
                ...
                {
                    pic::msg() << "Can't set sndbuf size" << ": " << sys_errlist[errno] << pic::log;
                }
            }
            ...
            {
                pic::msg() << "Can't setup mcast if" << ": " << sys_errlist[errno] << pic::hurl;
            }
        ...
        void send(unsigned long addr, const unsigned char *data, unsigned len)
        {
             ...
             if(s != (int)len)
             {
                pic::logmsg() << "Can't send multicast data on " << socket_.fd << ':' << (void *)this << ": " << sys_errlist[errno];
             }
        }
    ...
    struct recv_socket_t
    {
        recv_socket_t(unsigned spc, unsigned long local, bool loop): socket_(AF_INET,SOCK_DGRAM,0), spc_(spc), local_(local)
        {
            ...
            {
                pic::msg() << "Can't setup mcast socket" << ": " << sys_errlist[errno] << pic::hurl;
            }
            ...
            int rsz;
            socklen_t rszl = sizeof(rsz);

            if(getsockopt(socket_.fd,SOL_SOCKET, SO_RCVBUF, (char *)&rsz, &rszl)<0)
            {
                rsz=0;
            }
            ...
            if(rsz < BUFFER_SIZE)
            {
                ...
                {
                    pic::msg() << "Can't set rcvbuf size" << ": " << sys_errlist[errno] << pic::log;
                }
            }
            ...
            if(getsockopt(socket_.fd,SOL_SOCKET, SO_RCVBUF, (char *)&rsz, &rszl)>=0)
            {
                //pic::logmsg() << "recv buf size: " << rsz;
            }

            if(setsockopt(socket_.fd, SOL_SOCKET, SO_RCVBUF, (char *)&rsz, sizeof(rsz)) < 0)
            {
                pic::msg() << "Can't set rcvbuf size" << ": " << sys_errlist[errno] << pic::hurl;
            }
            ...
            if(bind(socket_.fd, (struct sockaddr*)&addr, sizeof(addr)))
            {
                pic::msg() << "Can't bind mcast socket" << ": " << sys_errlist[errno] << pic::hurl;
            }
        }
        ...
        int recv(unsigned long *addr, unsigned short *port, unsigned char *data, unsigned len)
        {
            ...
            if((l=recvfrom(socket_.fd, (char *)data, len, MSG_WAITALL|MSG_DONTWAIT, (struct sockaddr *)&a, &al)) < 0)
            {
                ...
                pic::msg() << "error receiving multicast data" << ": " << sys_errlist[errno] << pic::hurl;
            }
    ...
    struct pia_bkernel_clock_thread_t: public pic::thread_t
    {
        ...
        void thread_main()
        {
            ...
            std::cout << "Started bkernel clock thread with ID " << pic_current_threadid() << std::endl;
#endif
    ...
    struct pia_bkernel_proto_thread_t: public pic::thread_t
    {
        ...
        {
#ifdef DEBUG_DATA_ATOMICITY
            std::cout << "Started bkernel protothread with ID " << pic_current_threadid() << std::endl;
#endif
    ...
    struct netbase_t: pic::thread_t, checker_t
    {
        ...
        {
#ifdef DEBUG_DATA_ATOMICITY
            std::cout << "Started netbase thread with ID " << pic_current_threadid() << std::endl;
#endif
...
{
    impl_t(pic::nballocator_t *a, bool clock): netbase_t(a,clock),kevsock_(PF_SYSTEM,SOCK_RAW,SYSPROTO_EVENT),statsock_(AF_INET,SOCK_DGRAM,0)
    {
        setup_kev();
        run();
    }
    ...
    void setup_kev()
    {
        ...
        req.kev_subclass=KEV_INET_SUBCLASS;

        if(ioctl(kevsock_.fd,SIOCSKEVFILT,&req) < 0)
        {
            pic::msg() << "can't establish kernel event filter" << ": " << sys_errlist[errno] << pic::hurl;
        }
    ...
    virtual bool process_monitor(fd_set *r)
    {
        ...
        {
            pic::logmsg() << "network status change";
    ...
    virtual unsigned long interface_filter(const char *name)
    {
        ...
        if (ioctl(statsock_.fd, SIOCGIFMEDIA, (caddr_t)&ifmr) < 0)
        {
            pic::logmsg() << name << ": can't get media";
            return 0;
        }

        if((ifmr.ifm_current&IFM_ETHER)!=IFM_ETHER)
        {
            pic::logmsg() << name << ": not wired ether";
            return 0;
        }
