...
public:
    ...
    /** Creates a custom cursor from an image.
        ...
        @param scale    the scale at which the image data is compared to the final
                        dimensions at which it will be drawn, useful for HiDPI displays
    */
    MouseCursor (const Image& image, int hotSpotX, int hotSpotY, float scale = 1.f);
...
private:
    ...
    static void* createMouseCursorFromImage (const Image& image, int hotspotX, int hotspotY, float scale);
