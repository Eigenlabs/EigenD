...
{
    static NSImage* createNSImage (const Image& image, const float scale)
    {
        ...
        CGContextDrawImage (cg, CGRectMake (0, 0, image.getWidth()/scale, image.getHeight()/scale), imageRef);
    ...
    static void* createFromImage (const Image& image, float hotspotX, float hotspotY, float scale)
    {
        NSImage* im = createNSImage (image, scale);
...
void* MouseCursor::createMouseCursorFromImage (const Image& image, int hotspotX, int hotspotY, float scale)
{
    return MouseCursorHelpers::createFromImage (image, (float) hotspotX, (float) hotspotY, scale);
}
...
void* MouseCursor::createStandardMouseCursor (MouseCursor::StandardCursorType type)
{
    ...
    switch (type)
    {
        ...
        case NoCursor:              return createMouseCursorFromImage (Image (Image::ARGB, 8, 8, true), 0, 0, 1.f);
...
void* MouseCursor::createMouseCursorFromImage (const Image& image, int hotspotX, int hotspotY, float scale)  { return nullptr; }
