...
static void dump(piw::client_t *c)
{
    ...
    if(c->host_flags & PLG_SERVER_RTRANSIENT)
    {
        std::cout << "/rtx";
    }
    ...
    if(c->host_flags & PLG_SERVER_FAST)
    {
        ...
        c->set_sink(&fastdata);
        std::cout << '(' << fastdata.current(false) << ')';
    }
...
int main0(int ac, char **av)
{
    ...
    pia::scaffold_mt_t manager("bcat",1,pic::f_string_t(),pic::f_string_t(),false,false);
    ...
    piw::tsd_client(av[1],&client,false);
    entity.release();
    manager.wait();
    return 0;
}
