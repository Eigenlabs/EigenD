...
static int send_data_broadcast(lo_address a, lo_server from, char *data, const size_t data_len);
...
static int send_data(lo_address a, lo_server from, char *data, const size_t data_len)
{
    ...
    if (from) 
    {
        ...
        if (a->protocol == LO_UDP && lo_client_sockets.udp!=-1) 
        {
...
static int send_data_broadcast(lo_address a, lo_server from, char *data, const size_t data_len)
{
    ...
    // eigenlabs change: for the socket to send to a broadcast address
...
int lo_send_message_from_broadcast(lo_address a, lo_server from, const char *path, lo_message msg)
{
    const size_t data_len = lo_message_length(msg, path);
    char *data = lo_message_serialise(msg, path, NULL, NULL);
    
    // Send the message
    int ret = send_data_broadcast( a, from, data, data_len );
    
    // For TCP, retry once if it failed.  The first try will return
    // error if the connection was closed, so the second try will
    // attempt to re-open the connection.
    if (ret == -1 && a->protocol == LO_TCP)
        ret = send_data_broadcast( a, from, data, data_len );
...
int lo_send_message_broadcast(lo_address a, const char *path, lo_message msg)
{
    return lo_send_message_from_broadcast( a, NULL, path, msg );
}

#ifdef __GNUC__
int lo_send_internal_broadcast(lo_address t, const char *file, const int line,
                     const char *path, const char *types, ...)
#else
int lo_send_broadcast(lo_address t, const char *path, const char *types, ...)
#endif
{
    va_list ap;
    int ret;
    ...
    ret = lo_send_message_broadcast(t, path, msg);
    lo_message_free(msg);
