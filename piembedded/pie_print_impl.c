...
void pie_printmsg(const void *m_, unsigned l, int nl, piu_output_t w, void *wa)
{
    ...
    {   
        ...
        if(bt==BCTMTYPE_TREE_SET)
        {
            unsigned char n;

            while((o=pie_gettsetpath(m,l,&n))>0)
            {
                m+=o; l-=o;
        ...
        if(bt==BCTMTYPE_DATA_EVT || bt==BCTMTYPE_DATA_SET || bt==BCTMTYPE_FAST_EVT || bt==BCTMTYPE_IDNT_EVT)
        {
            unsigned df;
            unsigned short dl;
