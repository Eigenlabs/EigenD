...
struct printer_t: public  micro::active_t::delegate_t
{
    ...
    void kbd_raw(bool resync,const micro::active_t::rawkbd_t &kbd)
    {
...
int main(int ac, char **av)
{
    ...
    else
    {
        try
        {
            usbdev = pic::usbenumerator_t::find(BCTMICRO_USBVENDOR,BCTMICRO_USBPRODUCT).c_str();
        }
        catch(...)
        {
    ...
    micro::active_t loop(usbdev, &printer);
    loop.start();
