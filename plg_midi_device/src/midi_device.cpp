...
midi_device_t::impl_t::impl_t(piw::clockdomain_ctl_t *cd,  const piw::cookie_t &outputs) :
	root_t(0), up_(0),
	channel_(0),
	enable_notes_(true),enable_velocity_(true), velocity_sample_(4),
	update_period_(0),
	note_cc(-1),note_pressure_cc(-1),last_cc_note(-1),last_cc_note_pressure(0),
	high_res_cc_(false)
{
	for (unsigned k=0;k<MAX_DEVICE;k++)
	{
		poly_at_[k]=false;
		chan_at_[k]=false;
		pb_[k]=false;
...
void midi_device_t::impl_t::create_wires()
{
    ...
    {
    	kwires_[k] = pic::ref(new kwire_t(k+1,col,k+1,piw::pathtwo(OUT_KEY,k+1,0),this));
    }
...
void midi_device_t::impl_t::decoder_noteon(unsigned c, unsigned number, unsigned velocity)
{
	...
	kwires_[key]->channel_=channel;

	// MSH : currently velocity is 7 bit only
	kwires_[key]->updateKey(t,true,v);

	LOG_SINGLE(pic::logmsg() << "decoder_noteon key: " << key << " id : " << kwires_[key]->id_ << " ch " << kwires_[key]->channel_;)
}
...
void midi_device_t::impl_t::decoder_noteoff(unsigned c, unsigned number, unsigned velocity)
{
	...
	unsigned long long t=last_event_t_;
	unsigned key=number;
	kwires_[key]->channel_=0;
	kwires_[key]->updateKey(t,false,0);
}
...
void midi_device_t::impl_t::decoder_polypressure(unsigned c, unsigned number, unsigned value)
{
	...
	if(poly_at_[PRESSURE])	kwires_[key]->updateP(t,value);
	if(poly_at_[YAW])		kwires_[key]->updateY(t,value);
	if(poly_at_[ROLL])		kwires_[key]->updateR(t,value);
...
void midi_device_t::impl_t::decoder_channelpressure(unsigned c, unsigned value)
{
	...
	{
		if(kwires_[key]->channel_==channel)
		{
			if(chan_at_[PRESSURE])	kwires_[key]->updateP(t,value);
			if(chan_at_[YAW])		kwires_[key]->updateY(t,value);
			if(chan_at_[ROLL])		kwires_[key]->updateR(t,value);
		}
	}
...
void midi_device_t::impl_t::decoder_pitchbend(unsigned c, unsigned value)
{
	...
	for (unsigned key=0;key<MAX_KEYS;key++)
	{
		if(kwires_[key]->channel_==channel)
		{
			if(pb_[PRESSURE]) kwires_[key]->updateLsbP(t,lsb);
			if(pb_[YAW])	  kwires_[key]->updateLsbY(t,lsb);
			if(pb_[ROLL])	  kwires_[key]->updateLsbR(t,lsb);

			if(pb_[PRESSURE]) kwires_[key]->updateP(t,msb);
			if(pb_[YAW])	  kwires_[key]->updateY(t,msb);
			if(pb_[ROLL])	  kwires_[key]->updateR(t,msb);
		}
	}
...
void midi_device_t::impl_t::decoder_cc(unsigned c, unsigned number, unsigned value)
{
	...
	if(high_res_cc_)
	{
		for (unsigned key=0;key<MAX_KEYS;key++)
		{
			if(kwires_[key]->channel_==channel)
			{
				if(hires_cc_[PRESSURE]==number) kwires_[key]->updateLsbP(t,value);
				if(hires_cc_[YAW]==number)	  kwires_[key]->updateLsbY(t,value);
				if(hires_cc_[ROLL]==number)	  kwires_[key]->updateLsbR(t,value);
			}
		}
	...
	for (unsigned key=0;key<MAX_KEYS;key++)
	{
		if(kwires_[key]->channel_==channel)
		{
			if(cc_[PRESSURE]==number) kwires_[key]->updateP(t,value);
			if(cc_[YAW]==number)	  kwires_[key]->updateY(t,value);
			if(cc_[ROLL]==number)	  kwires_[key]->updateR(t,value);
		}
	}
	...
	if (note_cc==number)
	{
		...
		if (last_cc_note !=value)
		{
			...
			if(last_cc_note > 0)
			{
				...
				kwires_[last_cc_note]->channel_=0;
				kwires_[last_cc_note]->updateKey(t,false,0);
				last_cc_note=0;
			}
			...
			if(value>0)
			{
				...
				if (last_cc_note_pressure<=0)
				{
					...
					kwires_[value]->channel_=0;
					kwires_[value]->updateKey(t,false,0);
				}
				else
				{
					LOG_SINGLE(pic::logmsg() << "note on " << number << " v " << value << "last_cc_note_pressure " << last_cc_note_pressure;)
					kwires_[value]->channel_=channel;
					kwires_[value]->updateKey(t,true,last_cc_note_pressure);
				}
			}
	...
	if (note_pressure_cc==number)
	{
		...
		if(last_cc_note > 0)
		{
			...
			{
				kwires_[last_cc_note]->updateP(t,last_cc_note_pressure);
			}
		}
	}
}
...
{
	// TODO, determine output midi channel
	midi_output_->sendNote(t,1,note,vel);
}


static const unsigned midi_device_col_length[1]= { MAX_KEYS};
...
midi_wire_t::midi_wire_t(midi_device_t::impl_t  *p, const piw::event_data_source_t &es):
    ...
    root_(p),
    last_from_(0),
    led_converter_(1,midi_device_col_length),
	light_t_(0)
{
...
void midi_wire_t::updateLights(unsigned key,unsigned value)
{
	...
	switch (value)
	{
		case CLR_OFF: velocity=0;break;
		case CLR_RED: velocity=5;break;
		case CLR_GREEN: velocity=87;break;
		case CLR_ORANGE: velocity=124;break;
		case CLR_MIXED: velocity=3;break;
		default: velocity=1;
	}
	unsigned long long t=light_t_++;

	LOG_SINGLE(pic::logmsg() << "midi_wire_t::updateLights t=" << t << " n= " << note << " v= " << velocity;)

	root_->sendMidiNote(t,note,velocity);
}
...
void midi_output_wire_t::sendNote(unsigned long long t, unsigned channel,unsigned note, unsigned velocity)
{
	...
	LOG_SINGLE(pic::logmsg() << "midi_output_wire_t::update d0=" << std::hex << (unsigned)d0 << " d1=" << (unsigned)d1 << " d2=" << (unsigned)d2 << " len=3 time=" << std::dec << t;)
...
piw::data_t  midi_device_t::get_columnlen()
{
    ...
    courses = piw::tupleadd(courses, piw::makelong(MAX_KEYS,0));
    return courses;
}
