...
class Skini : public Stk
{
 public:
  ...
  struct Message { 
    ...
    pic::lckvector_t<StkFloat>::lcktype floatValues; /*!< The message values read as floats (values are type-specific). */
    pic::lckvector_t<long>::lcktype intValues;       /*!< The message values read as ints (number and values are type-specific). */
 ...
 protected:
  ...
  void tokenize( const std::string& str, pic::lckvector_t<std::string>::lcktype &tokens, const std::string& delimiters );
