...
MidiFileIn :: MidiFileIn( std::string fileName )
{
  ...
  if ( format_ == 1 && !usingTimeCode_ ) {
    pic::lckvector_t<unsigned char>::lcktype event;
...
unsigned long MidiFileIn :: getNextEvent( pic::lckvector_t<unsigned char>::lcktype *event, unsigned int track )
{
...
unsigned long MidiFileIn :: getNextMidiEvent( pic::lckvector_t<unsigned char>::lcktype *midiEvent, unsigned int track )
{
