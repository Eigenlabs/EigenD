...
class Atom(node.Server):
    ...
    def __init__(self,init=None,domain=None,policy=None,transient=False,rtransient=False,names=None,protocols=None,ordinal=None,fuzzy=None,creator=None,wrecker=None,pronoun=None,icon=None,container=None,ideals=None,bignode=False,dynlist=False):
        ...
        if fullname:
            self.set_property_string('name',fullname)
            self.set_property_string('cname',fullname)

        if pronoun is not None:
            self.set_property_string('pronoun',' '.join(namelist))

        if fuzzy is not None:
            self.set_property_string('fuzzy',fuzzy)
        ...
        if icon is not None:
            ...
            self.set_property_string('icon',ideal)

        if ideals is not None:
            self.set_property_string('ideals',ideals)
        ...
        if user_p:
            self.set_property_string('protocols',user_p)
    ...
    def set_property_string(self,key,value,allow_veto = False):
        self.set_property(key,piw.makestring(value,0),allow_veto)

    def get_property_string(self,key,default = ''):
        v = self.get_meta_data().as_dict_lookup(key)
        return v.as_string() if v.is_string() else default

    def set_property_termlist(self,key,value,allow_veto = False):
        self.set_property(key,piw.makestring(logic.render_termlist(value),0),allow_veto)
    ...
    def add_property_termlist(self,key,term,allow_veto = False):
        term = logic.parse_clause(term,nosubst=True)
        l = self.get_property_termlist(key)
        if term not in l: l.append(term)
        self.set_property_termlist(key,l,allow_veto)

    def del_property_termlist(self,key,term,allow_veto = False):
        term = logic.parse_clause(term,nosubst=True)
        l = self.get_property_termlist(key)
        if term in l: l.remove(term)
        self.set_property_termlist(key,l,allow_veto)
    ...
    def set_domain(self,domain):
        self.set_property_string('domain',str(domain))
