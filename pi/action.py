...
def adapt_callbackn(r1):
    r2 = async.Deferred()
    r2.original = r1

    def succeeded(result):
        r2.succeeded(*unmarshal(result))
    def failed(message):
        r2.failed(message or 'rpc error')

    r1.setCallback(succeeded).setErrback(failed)
    return r2

def adapt_callback1(r1):
    r2 = async.Deferred()
    r2.original = r1
