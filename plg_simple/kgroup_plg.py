...
class Output(agent.Agent):
    ...
    def enable(self,val):
        self.__agent.mode_selector.select(self.name-1,val)
        return True

    def plumb(self):
        n = self.name

        self.__agent.unplumb(n)
    ...
    def unplumb(self):
        ...
        self.__agent.selgate.clear_functor(piw.pathone(n,0))
    ...
    def server_opened(self):
        ...
        self.set_frelation('create(cnc("%s"),role(by,[instance("%s")]))' % (self.id(),self.__agent.id()))
...
class Agent(agent.Agent):
    def __init__(self, address, ordinal):
        agent.Agent.__init__(self, signature=version, names='kgroup', icon='plg_simple/kgroup.png',container=8,protocols='has_subsys',ordinal=ordinal)
        ...
        self.__private[1] = container.PersistentFactory(asserted=self.__inst_asserted,retracted=self.__inst_retracted,moved=self.__inst_moved)
        ...
        self.add_verb2(6,'create([un],None,role(None,[concrete,singular,issubject(create,[role(by,[cnc(~server)])])]))', self.__uncreate)
        ...
        self.add_verb2(10,'move([],None,role(None,[concrete,singular,issubject(create,[role(by,[cnc(~server)])])]),role(to,[numeric]))', self.__move)
    ...
    def __upstream(self,k):
        ...
        #for k,v in self.iter_subsys_items(): v.lmix.set_size(size)
    ...
    def __inst_moved(self,index,oval,nval,state):
        state.unplumb()
        state.name = int(nval)
        state.set_ordinal(int(nval))
        state.plumb()
    ...
    def __create(self,subj,name):
        ...
        for k,v in self.iter_subsys_items():
            if v.name==name:
                #return async.failure('output %d exists' % name)
                return async.success(errors.already_exists('output %d' % name,'create'))

        g = self.__private[1].assert_state(str(name))
        return action.concrete_return(g.id())

    def unplumb(self,name):
        for k,v in self.iter_subsys_items():
            if v.name==name:
                v.unplumb()
                return
    ...
    def __uncreate(self,subj,grp):
        ...
        if self.__private[1].retract_state(lambda s,g: g.id()==a):
            return async.success(action.removed_return(a))
        #return async.failure('output not found')
        return async.success(errors.doesnt_exist('output','un create'))

    def __move(self,subj,src,dst):
        a = action.concrete_object(src)
        name = int(action.abstract_wordlist(dst)[0])
        ...
        if self.__private[1].move_state(name,lambda s,g: g.id()==a):
